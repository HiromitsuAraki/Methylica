...
### Parameter setting
Following data upload, *Methylica* requests its users to select a platform (Sequence-based or Infinium methylation array), species with its reference genome version, genomic elements to be analyzed (CpG island, CpG island shores, gene body, first intron and promoter), and k or the number of ICs (minimum = 2; maximum = the number of samples). When users select CpG island shores, which are 2kbp flanking regions on each side of CpG island, genomic coordinates of input data matrix have to be identical to CpG islands. We recommend that data matrix to be analyzed is generated by  [*MatrixMaker*](https://github.com/HiromitsuAraki/MatrixMaker). *Methylica* provides a default setting of k, defined as the first k components whose cumulative contribution ratio exceeds 80% in principal component analysis. When users select all parameters, users need to press "Run" button to start analysis.
