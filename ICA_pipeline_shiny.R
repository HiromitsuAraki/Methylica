...
run_ICA<-function(dm,Method,IC){
  ...
  if (Method == "JADE"){ 
    resICA     <- runICA(X=dm_data, nbComp=IC, method=Method, maxit=100000)
  }
  
  if (Method == "fastICA"){
    resICA     <- runICA(X=dm_data, nbComp=IC, method=Method, maxit=100000, alg.typ="parallel")
...
##PC >=80
run_ICA_PC80<-function(dm,Method){
  gc()
  gc()
...
make_ICA_heatmap_shiny<-function(resICA,sampleList,GenomicFeature){
  ...
  if (GenomicFeature==3){Loci="Promoter"}
...
generate_region2loadings<-function(resICA,Genome,Feature){
  ...
  if (Feature==1){
    ...
    if (Genome==1){CGI_closestTSS=read.table("~/hg38_cgi2closestTSS",header=T,row.names=1)}
    if (Genome==2){CGI_closestTSS=read.table("~/hg19_cgi2closestTSS",header=T,row.names=1)}
    if (Genome==3){CGI_closestTSS=read.table("~/mm10_cgi2closestTSS",header=T,row.names=1)}
    if (Genome==4){CGI_closestTSS=read.table("~/mm9_cgi2closestTSS",header=T,row.names=1)}
