...
generate_region2loadings<-function(resICA,Genome,Feature){
  ...
  if (Feature==1){
    ...
    if (Genome==3){CGI_closestTSS=read.table("HiromitsuAraki/Methylica/annotation/mm10_cgi2closestTSS_nr",header=T,row.names=1)}
    if (Genome==4){CGI_closestTSS=read.table("HiromitsuAraki/Methylica/annotation/mm9_cgi2closestTSS_nr",header=T,row.names=1)}
