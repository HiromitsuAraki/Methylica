...
generate_region2loadings<-function(resICA,Genome,Feature){
  ...
  if (Feature==1){
    ...
    if (Genome==1){CGI_closestTSS=read.table("HiromitsuAraki/Methylica/annotation/hg38_cgi2closestTSS_nr",header=T,row.names=1)}
    if (Genome==2){CGI_closestTSS=read.table("HiromitsuAraki/Methylica/annotation/hg19_cgi2closestTSS_nr",header=T,row.names=1)}
    if (Genome==3){CGI_closestTSS=read.table("HiromitsuAraki/Methylica/annotation/annotation/mm10_cgi2closestTSS_nr",header=T,row.names=1)}
    if (Genome==4){CGI_closestTSS=read.table("HiromitsuAraki/Methylica/annotation/annotation/mm9_cgi2closestTSS_nr",header=T,row.names=1)}
...
generate_highLF_target_shiny<-function(resICA,TargetIC,Zscore,Genome,Feature){
  ...
  if (length(target_dm)>1){
    ...
    if (Feature==1){
      ...
      if (Genome==1){CGI_closestTSS=read.table("HiromitsuAraki/Methylica/annotation/annotation/hg38_cgi2closestTSS_nr",header=T,row.names=1)}
      if (Genome==2){CGI_closestTSS=read.table("HiromitsuAraki/Methylica/annotation/annotation/hg19_cgi2closestTSS_nr",header=T,row.names=1)}
      if (Genome==3){CGI_closestTSS=read.table("HiromitsuAraki/Methylica/annotation/annotation/mm10_cgi2closestTSS_nr",header=T,row.names=1)}
      if (Genome==4){CGI_closestTSS=read.table("HiromitsuAraki/Methylica/annotation/annotation/mm9_cgi2closestTSS_nr",header=T,row.names=1)}
